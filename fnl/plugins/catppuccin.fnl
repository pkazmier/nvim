...
(with-later! ; catppuccin
  ...
  (catppuccin.setup
    {:default_integrations false
     ...
     ;; "@..." keys are set via `tset` because fnlfmt rewrites
     ;; literal "@..." table keys into :@... keywords, which
     ;; hotpot's fennel cannot parse.
     :highlight_overrides 
     {:all (fn [colors]
             (local overrides
             ...
             ;; ;; block quotes
             (tset overrides "@markup.quote" {:fg colors.maroon :style [:italic]})
             overrides)
      :mocha (fn [colors]
               (local overrides
                      {:Headline                       {:style [:bold]}
                       ...
                       :MsgArea                        {:fg colors.overlay2} :CmpItemAbbrMatch {:fg colors.green :style [:bold]}
               ...
               (each [_ hl (ipairs [:Headline :rainbow])]
                 (each [i c (ipairs [:green :sapphire :mauve :peach :red :yellow])]
                   (tset overrides (.. hl i) {:fg (. colors c) :style [:bold]})))
               overrides)
      ...
      :macchiato (fn [colors]
                   (local overrides
                          {:CurSearch               {:bg colors.peach}
                           ...
                           :MiniTablineVisible             {:fg colors.overlay1 :bg colors.surface0 :style [:bold]}})
                   (each [_ hl (ipairs [:Headline :rainbow])]
                     (each [i c (ipairs [:blue :pink :lavender :green :peach :flamingo])]
                       (tset overrides (.. hl i) {:fg (. colors c) :style [:bold]})))
                   (tset overrides "@string.special.symbol" {:link :Special})
                   (tset overrides "@constructor.lua" {:fg colors.pink})
                   overrides)}
     :color_overrides 
     {:macchiato 
      {:rosewater "#F5B8AB"
