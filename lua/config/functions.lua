...
function _G.pp(...) vim.notify(vim.iter({ ... }):map(vim.inspect):join(", ")) end
...
function M.choose(choices) return choices[math.random(1, #choices)] end
...
function M.new_scratch_buffer() vim.api.nvim_win_set_buf(0, vim.api.nvim_create_buf(true, true)) end

-- ---------------------------------------------------------------------------
-- Re-map Binding
-- ---------------------------------------------------------------------------

function M.remap(mode, lhs_from, lhs_to)
  local keymap = vim.fn.maparg(lhs_from, mode, false, true)
  local rhs = keymap.callback or keymap.rhs
  assert(rhs, "Could not remap from " .. lhs_from .. " to " .. lhs_to)
...
--
--     local clues = require("config.functions").gen_hydra_brackets({ "d" }, {
...
function M.gen_hydra_brackets(suffixes, replacements)
  local clues = {}
...
function H.capitalize(w) return w:sub(1, 1):upper() .. w:sub(2) end
